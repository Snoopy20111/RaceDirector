...
func add_point_inspector(index):
	...
	if curve is Curve3D:
		i.set_point_vals_3d(curve.get_point_position(index).x, curve.get_point_position(index).y, curve.get_point_left_tangent(index), curve.get_point_right_tangent(index), curve.get_point_tilt(index))
